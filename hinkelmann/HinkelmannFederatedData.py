...
class HKClientData(client_data.ClientData):

    def __init__(self, filepath, feature_names, skiprows=0, labelname='Label'):
    ...
    def _create_dataset(self):
        ...
        df_test = pd.read_csv(
            self._filepath,
            names=[str(self.feature_names), str(self.label_name)],
        ...
        df = pd.read_csv(
            self._filepath,
            names=[str(self.feature_names), str(self.label_name)],
            engine='c', dtype=all_cols, skiprows=self.skiprows, low_memory=False)
